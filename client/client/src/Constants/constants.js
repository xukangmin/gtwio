export const gConstants = {
    API_ROOT: 'http://localhost:10010',
    SERVER_ROOT: 'http://localhost',
    SUCCESS: 'ALERT_SUCCESS',
    ERROR: 'ALERT_ERROR',
    CLEAR: 'ALERT_CLEAR',
    INFO: 'ALERT_INFO',

    REGISTER_REQUEST: 'USERS_REGISTER_REQUEST',
    REGISTER_SUCCESS: 'USERS_REGISTER_SUCCESS',
    REGISTER_FAILURE: 'USERS_REGISTER_FAILURE',

    LOGIN_REQUEST: 'USERS_LOGIN_REQUEST',
    LOGIN_SUCCESS: 'USERS_LOGIN_SUCCESS',
    LOGIN_FAILURE: 'USERS_LOGIN_FAILURE',

    RESET_REQUEST: 'USERS_RESET_REQUEST',
    RESET_SUCCESS: 'USERS_RESET_SUCCESS',
    RESET_FAILURE: 'USERS_RESET_FAILURE',

    LOGOUT: 'USERS_LOGOUT',

    USERS_GET_ALL_REQUEST: 'USERS_GET_ALL_REQUEST',
    USERS_GET_ALL_SUCCESS: 'USERS_GET_ALL_SUCCESS',
    USERS_GET_ALL_FAILURE: 'USERS_GET_ALL_FAILURE',

    DELETE_REQUEST: 'USERS_DELETE_REQUEST',
    DELETE_SUCCESS: 'USERS_DELETE_SUCCESS',
    DELETE_FAILURE: 'USERS_DELETE_FAILURE',

    // ********* ASSETS ************
    GET_ASSETS_REQUEST: 'GET_ASSETS_REQUEST',
    GET_ASSETS_SUCCESS: 'GET_ASSETS_SUCCESS',
    GET_ASSETS_FAILURE: 'GET_ASSETS_FAILURE',

    GET_ASSET_REQUEST: 'GET_ASSET_REQUEST',
    GET_ASSET_SUCCESS: 'GET_ASSET_SUCCESS',
    GET_ASSET_FAILURE: 'GET_ASSET_FAILURE',

    GET_ASSET_CONFIG_REQUEST: 'GET_ASSET_CONFIG_REQUEST',
    GET_ASSET_CONFIG_SUCCESS: 'GET_ASSET_CONFIG_SUCCESS',
    GET_ASSET_CONFIG_FAILURE: 'GET_ASSET_CONFIG_FAILURE',

    UPDATE_ASSET_CONFIG_REQUEST: 'UPDATE_ASSET_CONFIG_REQUEST',
    UPDATE_ASSET_CONFIG_SUCCESS: 'UPDATE_ASSET_CONFIG_SUCCESS',
    UPDATE_ASSET_CONFIG_FAILURE: 'UPDATE_ASSET_CONFIG_FAILURE',

    GET_CONFIG_BY_ASSET_ID_REQUEST: 'GET_CONFIG_BY_ASSET_ID_REQUEST',
    GET_CONFIG_BY_ASSET_ID_SUCCESS: 'GET_CONFIG_BY_ASSET_ID_SUCCESS',
    GET_CONFIG_BY_ASSET_ID_FAILURE: 'GET_CONFIG_BY_ASSET_ID_FAILURE',

    GET_ASSET_DATA_REQUEST: 'GET_ASSET_DATA_REQUEST',
    GET_ASSET_DATA_SUCCESS: 'GET_ASSET_DATA_SUCCESS',
    GET_ASSET_DATA_FAILURE: 'GET_ASSET_DATA_FAILURE',

    GET_ASSET_TAG_REQUEST: 'GET_ASSET_TAG_REQUEST',
    GET_ASSET_TAG_SUCCESS: 'GET_ASSET_TAG_SUCCESS',
    GET_ASSET_TAG_FAILURE: 'GET_ASSET_TAG_FAILURE',

    GET_DEVICES_BY_ASSET_REQUEST: 'GET_DEVICES_BY_ASSET_REQUEST',
    GET_DEVICES_BY_ASSET_SUCCESS: 'GET_DEVICES_BY_ASSET_SUCCESS',
    GET_DEVICES_BY_ASSET_FAILURE: 'GET_DEVICES_BY_ASSET_FAILURE',

    GET_TIME_RANGE_BY_ASSET_REQUEST: 'GET_TIME_RANGE_BY_ASSET_REQUEST',
    GET_TIME_RANGE_BY_ASSET_SUCCESS: 'GET_TIME_RANGE_BY_ASSET_SUCCESS',
    GET_TIME_RANGE_BY_ASSET_FAILURE: 'GET_TIME_RANGE_BY_ASSET_FAILURE',

    ADD_ASSET_REQUEST: 'ADD_ASSET_REQUEST',
    ADD_ASSET_SUCCESS: 'ADD_ASSET_SUCCESS',
    ADD_ASSET_FAILURE: 'ADD_ASSET_FAILURE',

    ADD_ASSET_BY_CONFIG_REQUEST: 'ADD_ASSET_BY_CONFIG_REQUEST',
    ADD_ASSET_BY_CONFIG_SUCCESS: 'ADD_ASSET_BY_CONFIG_SUCCESS',
    ADD_ASSET_BY_CONFIG_FAILURE: 'ADD_ASSET_BY_CONFIG_FAILURE',

    ADD_ASSET_BY_CONFIG_FILE_REQUEST: 'ADD_ASSET_BY_CONFIG_FILE_REQUEST',
    ADD_ASSET_BY_CONFIG_FILE_SUCCESS: 'ADD_ASSET_BY_CONFIG_FILE_SUCCESS',
    ADD_ASSET_BY_CONFIG_FILE_FAILURE: 'ADD_ASSET_BY_CONFIG_FILE_FAILURE',

    DELETE_ASSET_REQUEST: 'DELETE_ASSET_REQUEST',
    DELETE_ASSET_SUCCESS: 'DELETE_ASSET_SUCCESS',
    DELETE_ASSET_FAILURE: 'DELETE_ASSET_FAILURE',

    UPDATE_ASSET_REQUEST: 'UPDATE_ASSET_REQUEST',
    UPDATE_ASSET_SUCCESS: 'UPDATE_ASSET_SUCCESS',
    UPDATE_ASSET_FAILURE: 'UPDATE_ASSET_FAILURE',

    GET_BASELINES_REQUEST: 'GET_BASELINES_REQUEST',
    GET_BASELINES_SUCCESS: 'GET_BASELINES_SUCCESS',
    GET_BASELINES_FAILURE: 'GET_BASELINES_FAILURE',

    ADD_BASELINE_REQUEST: 'ADD_BASELINE_REQUEST',
    ADD_BASELINE_SUCCESS: 'ADD_BASELINE_SUCCESS',
    ADD_BASELINE_FAILURE: 'ADD_BASELINE_FAILURE',

    DELETE_BASELINE_REQUEST: 'DELETE_BASELINE_REQUEST',
    DELETE_BASELINE_SUCCESS: 'DELETE_BASELINE_SUCCESS',
    DELETE_BASELINE_FAILURE: 'DELETE_BASELINE_FAILURE',

    UPDATE_BASELINE_REQUEST: 'UPDATE_BASELINE_REQUEST',
    UPDATE_BASELINE_SUCCESS: 'UPDATE_BASELINE_SUCCESS',
    UPDATE_BASELINE_FAILURE: 'UPDATE_BASELINE_FAILURE',

    GET_TIME_INTERVALS_REQUEST: 'GET_TIME_INTERVALS_REQUEST',
    GET_TIME_INTERVALS_SUCCESS: 'GET_TIME_INTERVALS_SUCCESS',
    GET_TIME_INTERVALS_FAILURE: 'GET_TIME_INTERVALS_FAILURE',

    ADD_TIME_INTERVAL_REQUEST: 'ADD_TIME_INTERVAL_REQUEST',
    ADD_TIME_INTERVAL_SUCCESS: 'ADD_TIME_INTERVAL_SUCCESS',
    ADD_TIME_INTERVAL_FAILURE: 'ADD_TIME_INTERVAL_FAILURE',

    DELETE_TIME_INTERVAL_REQUEST: 'DELETE_TIME_INTERVAL_REQUEST',
    DELETE_TIME_INTERVAL_SUCCESS: 'DELETE_TIME_INTERVAL_SUCCESS',
    DELETE_TIME_INTERVAL_FAILURE: 'DELETE_TIME_INTERVAL_FAILURE',

    UPDATE_TAG_TIME_INTERVAL_REQUEST: 'UPDATE_TAG_TIME_INTERVAL_REQUEST',
    UPDATE_TAG_TIME_INTERVAL_SUCCESS: 'UPDATE_TAG_TIME_INTERVAL_SUCCESS',
    UPDATE_TAG_TIME_INTERVAL_FAILURE: 'UPDATE_TAG_TIME_INTERVAL_FAILURE',

    REMOVE_DEVICE_REQUEST: 'REMOVE_DEVICE_REQUEST',
    REMOVE_PARAMETER_REQUEST: 'REMOVE_PARAMETER_REQUEST',
    ADD_DEVICE_TO_CONFIG_REQUEST: 'ADD_DEVICE_TO_CONFIG_REQUEST',
    ADD_PARAMETER_TO_CONFIG_REQUEST: 'ADD_PARAMETER_TO_CONFIG_REQUEST',
    // ****************************

    // ********* DEVICE ************
    GET_DEVICES_REQUEST: 'GET_DEVICES_REQUEST',
    GET_DEVICES_SUCCESS: 'GET_DEVICES_SUCCESS',
    GET_DEVICES_FAILURE: 'GET_DEVICES_FAILURE',

    GET_DEVICE_REQUEST: 'GET_DEVICE_REQUEST',
    GET_DEVICE_SUCCESS: 'GET_DEVICE_SUCCESS',
    GET_DEVICE_FAILURE: 'GET_DEVICE_FAILURE',

    GET_DEVICE_BY_SN_REQUEST: 'GET_DEVICE_BY_SN_REQUEST',
    GET_DEVICE_BY_SN_SUCCESS: 'GET_DEVICE_BY_SN_SUCCESS',
    GET_DEVICE_BY_SN_FAILURE: 'GET_DEVICE_BY_SN_FAILURE',

    ADD_DEVICE_REQUEST: 'ADD_DEVICE_REQUEST',
    ADD_DEVICE_SUCCESS: 'ADD_DEVICE_SUCCESS',
    ADD_DEVICE_FAILURE: 'ADD_DEVICE_FAILURE',

    DELETE_DEVICE_REQUEST: 'DELETE_DEVICE_REQUEST',
    DELETE_DEVICE_SUCCESS: 'DELETE_DEVICE_SUCCESS',
    DELETE_DEVICE_FAILURE: 'DELETE_DEVICE_FAILURE',

    UPDATE_DEVICE_REQUEST: 'UPDATE_DEVICE_REQUEST',
    UPDATE_DEVICE_SUCCESS: 'UPDATE_DEVICE_SUCCESS',
    UPDATE_DEVICE_FAILURE: 'UPDATE_DEVICE_FAILURE',
    // ****************************
    
    // ********* PARAMETER ************
    GET_PARAMETER_REQUEST: 'GET_PARAMETER_REQUEST',
    GET_PARAMETER_SUCCESS: 'GET_PARAMETER_SUCCESS',
    GET_PARAMETER_FAILURE: 'GET_PARAMETER_FAILURE',

    GET_PARAMETERS_REQUEST: 'GET_PARAMETERS_REQUEST',
    GET_PARAMETERS_SUCCESS: 'GET_PARAMETERS_SUCCESS',
    GET_PARAMETERS_FAILURE: 'GET_PARAMETERS_FAILURE',

    ADD_PARAMETER_REQUEST: 'ADD_PARAMETER_REQUEST',
    ADD_PARAMETER_SUCCESS: 'ADD_PARAMETER_SUCCESS',
    ADD_PARAMETER_FAILURE: 'ADD_PARAMETER_FAILURE',

    UPDATE_PARAMETER_REQUEST: 'UPDATE_PARAMETER_REQUEST',
    UPDATE_PARAMETER_SUCCESS: 'UPDATE_PARAMETER_SUCCESS',
    UPDATE_PARAMETER_FAILURE: 'UPDATE_PARAMETER_FAILURE',

    DELETE_PARAMETER_REQUEST: 'DELETE_PARAMETER_REQUEST',
    DELETE_PARAMETER_SUCCESS: 'DELETE_PARAMETER_SUCCESS',
    DELETE_PARAMETER_FAILURE: 'DELETE_PARAMETER_FAILURE',
    // ****************************

    // ********* DATA ************
    GET_DATA_REQUEST: 'GET_DATA_REQUEST',
    GET_DATA_SUCCESS: 'GET_DATA_SUCCESS',
    GET_DATA_FAILURE: 'GET_DATA_FAILURE',

    GET_TAG_DATA_REQUEST: 'GET_TAG_DATA_REQUEST',
    GET_TAG_DATA_SUCCESS: 'GET_TAG_DATA_SUCCESS',
    GET_TAG_DATA_FAILURE: 'GET_TAG_DATA_FAILURE',

    GET_PARAMETER_DATA_REQUEST: 'GET_PARAMETER_DATA_REQUEST',
    GET_PARAMETER_DATA_SUCCESS: 'GET_PARAMETER_DATA_SUCCESS',
    GET_PARAMETER_DATA_FAILURE: 'GET_PARAMETER_DATA_FAILURE',

    GET_ASSET_DATA_REQUEST: 'GET_ASSET_DATA_REQUEST',
    GET_ASSET_DATA_SUCCESS: 'GET_ASSET_DATA_SUCCESS',
    GET_ASSET_DATA_FAILURE: 'GET_ASSET_DATA_FAILURE',

    GET_SERIAL_NUMBER_DATA_REQUEST: 'GET_SERIAL_NUMBER_DATA_REQUEST',
    GET_SERIAL_NUMBER_DATA_SUCCESS: 'GET_SERIAL_NUMBER_DATA_SUCCESS',
    GET_SERIAL_NUMBER_DATA_FAILURE: 'GET_SERIAL_NUMBER_DATA_FAILURE',

    ADD_DATA_REQUEST: 'ADD_DATA_REQUEST',
    ADD_DATA_SUCCESS: 'ADD_DATA_SUCCESS',
    ADD_DATA_FAILURE: 'ADD_DATA_FAILURE',

    DELETE_DATA_REQUEST: 'DELETE_DATA_REQUEST',
    DELETE_DATA_SUCCESS: 'DELETE_DATA_SUCCESS',
    DELETE_DATA_FAILURE: 'DELETE_DATA_FAILURE',

    GET_DATA_FOR_BASELINE_SELECTION_REQUEST: 'GET_DATA_FOR_BASELINE_SELECTION_REQUEST',
    GET_DATA_FOR_BASELINE_SELECTION_SUCCESS: 'GET_DATA_FOR_BASELINE_SELECTION_SUCCESS',
    GET_DATA_FOR_BASELINE_SELECTION_FAILURE: 'GET_DATA_FOR_BASELINE_SELECTION_FAILURE',
    // ****************************

    // ********* DASHBOARD ************
    GET_DASHBOARD_REQUEST: 'GET_DASHBOARD_REQUEST',
    GET_DASHBOARD_SUCCESS: 'GET_DASHBOARD_SUCCESS',
    GET_DASHBOARD_FAILURE: 'GET_DASHBOARD_FAILURE',

    ADD_DASHBOARD_REQUEST: 'ADD_DASHBOARD_REQUEST',
    ADD_DASHBOARD_SUCCESS: 'ADD_DASHBOARD_SUCCESS',
    ADD_DASHBOARD_FAILURE: 'ADD_DASHBOARD_FAILURE',

    UPDATE_DASHBOARD_REQUEST: 'UPDATE_DASHBOARD_REQUEST',
    UPDATE_DASHBOARD_SUCCESS: 'UPDATE_DASHBOARD_SUCCESS',
    UPDATE_DASHBOARD_FAILURE: 'UPDATE_DASHBOARD_FAILURE',

    DELETE_DASHBOARD_REQUEST: 'DELETE_DASHBOARD_REQUEST',
    DELETE_DASHBOARD_SUCCESS: 'DELETE_DASHBOARD_SUCCESS',
    DELETE_DASHBOARD_FAILURE: 'DELETE_DASHBOARD_FAILURE',
    // ****************************

    // ********* VARIABLE ************
    GET_VARIABLE_REQUEST: 'GET_VARIABLE_REQUEST',
    GET_VARIABLE_SUCCESS: 'GET_VARIABLE_SUCCESS',
    GET_VARIABLE_FAILURE: 'GET_VARIABLE_FAILURE',

    ADD_VARIABLE_REQUEST: 'ADD_VARIABLE_REQUEST',
    ADD_VARIABLE_SUCCESS: 'ADD_VARIABLE_SUCCESS',
    ADD_VARIABLE_FAILURE: 'ADD_VARIABLE_FAILURE',

    DELETE_VARIABLE_REQUEST: 'DELETE_VARIABLE_REQUEST',
    DELETE_VARIABLE_SUCCESS: 'DELETE_VARIABLE_SUCCESS',
    DELETE_VARIABLE_FAILURE: 'DELETE_VARIABLE_FAILURE',
    // ****************************
};
